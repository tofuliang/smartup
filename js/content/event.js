...
chrome.runtime.onMessage.addListener(function(message,sender,sendResponse) {
	...
	if(message.type=="paste"){
		if(sue.startEle&&typeof sue.startEle.value=="string"){
			sue.startEle.value+=message.value||"";
		}
		sendResponse({type:message.type,message:true});
	}
	if(message.type=="copylnktxt"){
		navigator.clipboard&&navigator.clipboard.writeText?navigator.clipboard.writeText(message.value||""):null;
		sendResponse({type:message.type,message:true});
	}
	if(message.type=="copylnkaslnk"){
		navigator.clipboard&&navigator.clipboard.writeText?navigator.clipboard.writeText(message.value||""):null;
		sendResponse({type:message.type,message:true});
	}
	if(message.type=="copytabele"){
		navigator.clipboard&&navigator.clipboard.writeText?navigator.clipboard.writeText(message.value||""):null;
		sendResponse({type:message.type,message:true});
	}
	if(message.type=="copyimgurl"){
		navigator.clipboard&&navigator.clipboard.writeText?navigator.clipboard.writeText(message.value||""):null;
		sendResponse({type:message.type,message:true});
	}
	if(message.type=="copyimg"){
		(async function(){
			try{
				var response=await fetch(message.value||"");
				var blob=await response.blob();
				const item=new ClipboardItem({"image/png":blob});
				await navigator.clipboard.write([item]);
				sendResponse({type:message.type,message:true});
			}catch(e){
				sendResponse({type:message.type,message:false});
			}
		})();
		return true;
	}
	if(message.type=="closeapps"){
		var apps=document.querySelectorAll("smartup.su_apps");
		for(var i=0;i<apps.length;i++){
			apps[i].remove();
		}
		sendResponse({type:message.type,message:true});
	}
	if(message.type=="icon"||message.type=="pop"){
		sue.selEle={};
		sue.selEle.txt=window.getSelection().toString();
